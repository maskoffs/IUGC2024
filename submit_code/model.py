...
class U_Net(nn.Module):
    ...
    def forward(self, x,isLabeled):
        ...
        seg = None
        seg_flag = True if cls else False
        if cls and isLabeled:
            d5 = self.Up5(e5)
...
class model:
    ...
    def predict(self, X,isLabeled):
        ...
        """
        # self.model.eval()
        image = torch.tensor(X, dtype=torch.float).unsqueeze(0)  # (1,3,512,512)

        cls, seg, seg_flag = self.model(image,isLabeled)  # cls:(1,2)  seg:(1,3,512,512)  seg_flag: based on classification result by model
